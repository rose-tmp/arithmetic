 ...
 * @author - ZwZ
 * @date - 2020/3/2 - 17:33
 * @Description:扑克牌顺子 LL今天心情特别好, 因为他去买了一副扑克牌, 发现里面居然有2个大王, 2个小王
 * (一副牌原本是54张^_^)...他随机从中抽出了5张牌,
 ...
 */
public class Day28isContinuous {
    public boolean isContinuous(int[] numbers) {
        ...
        for (int i = count0; i < numbers.length - 1; i++) {
            ...
            if (numbers[i] == numbers[i + 1])
                return false;
            countInterval += numbers[i + 1] - numbers[i] - 1;//间隙个数
        }
        if (count0 >= countInterval)
            return true;
        return false;
    }
    ...
    public static void main(String[] args) {
        Day28isContinuous isContinuous = new Day28isContinuous();
        int[] arr = {1,3,0,5,0};
